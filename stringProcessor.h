#ifndef STRING_PROCESSOR
#define STRING_PROCESSOR

// 1. СИСТЕМНІ ВКЛЮЧЕННЯ
#include <stdint.h>
#include <stdbool.h>
#include <malloc.h>
#include <ctype.h>
#include <stdio.h>

#define STRING_MAX 512ull

// 2. ПРОТОТИПИ (Оголошення)
// *** УВАГА: Прибираємо const перед типом, якщо він є у тілі функції ***
const int32_t get_str(const char* msg, char* str, const int32_t limit);
const size_t strlenn(const char* str);
void strcopy(char* fStr, char* sStr, size_t until);
int32_t strcmpp(const char* fStr, const char* sStr);
char* strcatt(char* fStr, const char* sStr);
bool is_string_valid(char* str);
char* process_string(char* str);

// 3. ВКЛЮЧЕННЯ ЗАЛЕЖНИХ ФАЙЛІВ ПІСЛЯ ПРОТОТИПІВ (для уникнення циклу)
#include "sort.h" 

// 4. ТІЛА ФУНКЦІЙ (Реалізація)
// Реалізація get_str
const int32_t get_str(const char* msg, char* str, const int32_t limit)
{
    printf("%s", msg);
    if (fgets(str, limit, stdin) == NULL)
    {
        return -1;
    }
    size_t len = strlenn(str);
    if (len > 0 && str[len - 1] == '\n')
    {
        str[len - 1] = '\0';
        return (int32_t)(len - 1);
    }
    return (int32_t)len;
}

// Реалізація strlenn
const size_t strlenn(const char* str)
{
    size_t length = 0;
    while (str[length] != '\0')
    {
        length++;
    }
    return length;
}

// Реалізація strcopy
void strcopy(char* fStr, char* sStr, size_t until)
{
    size_t i = 0;
    while (i < until && sStr[i] != '\0')
    {
        fStr[i] = sStr[i];
        i++;
    }
    fStr[i] = '\0';
}

// Реалізація strcmpp
int32_t strcmpp(const char* fStr, const char* sStr)
{
    while (*fStr && (*fStr == *sStr))
    {
        fStr++;
        sStr++;
    }
    return *(const unsigned char*)fStr - *(const unsigned char*)sStr;
}

// Реалізація strcatt
char* strcatt(char* fStr, const char* sStr)
{
    char* ptr = fStr + strlenn(fStr);
    while (*sStr != '\0')
    {
        *ptr++ = *sStr++;
    }
    *ptr = '\0';
    return fStr;
}

// Реалізація is_string_valid
bool is_string_valid(char* str)
{
    bool isThereString = false;
    bool isThereColon = false;
    const size_t size = strlenn(str);
    for (size_t i = 0; i < size; i++)
    {
        if ((str[i] == ':' || str[i] == ';' || str[i] == ',') && i == 0ull)
        {
            return false;
        }
        if (str[i] == ':')
        {
            isThereColon = true;
        }
        else if (isThereColon)
        {
            if (str[i] == ',')
            {
                if (!isalpha(str[i - 1ull]) && str[i - 1ull] != ' ')
                {
                    return false;
                }
                else
                {
                    isThereString = true;
                }
            }
            if (str[i] == ';')
            {
                if (!isThereString || (!isalpha(str[i - 1ull]) && str[i - 1ull] != ' '))
                {
                    return false;
                }
                else
                {
                    isThereColon = false;
                }
            }
            if (str[i] == ':')
            {
                return false;
            }
        }
        else
        {
            isThereString = false;
        }
    }
    return true;
}

// Реалізація process_string
char* process_string(char* str)
{
    const size_t length = strlenn(str);
    char* result = (char*)calloc(length, sizeof(char));
    for (size_t i = 0; i < length; i++)
    {
        if (str[i] != ':')
        {
            char temp[2] = { str[i], '\0' };
            strcatt(result, temp);
            continue;
        }
        if (str[i + 1] != ' ')
        {
            return NULL;
        }
        strcatt(result, ": ");
        const size_t strsPreCnt = 100;
        char** arr = (char**)calloc(strsPreCnt, sizeof(char*));

        // Звільнення пам'яті для arr (не забуваємо про C6011)
        if (arr == NULL) return NULL; // Погана обробка, але необхідна

        for (size_t strsCnt = 0; strsCnt < strsPreCnt; strsCnt++)
        {
            arr[strsCnt] = (char*)calloc(STRING_MAX, sizeof(char));
        }
        for (size_t j = i + 2ull, strRealCnt = 0ull; j < length; j++)
        {
            if (str[j] == ',')
            {
                ++strRealCnt;
                ++j;
            }
            else if (str[j] == ';')
            {
                quick_sort(arr, ++strRealCnt); // Виклик функції сортування
                for (size_t k = 0; k < strRealCnt; k++)
                {
                    strcatt(result, arr[k]);
                    if (k == strRealCnt - 1ull)
                    {
                        strcatt(result, ";");
                    }
                    else
                    {
                        strcatt(result, ", ");
                    }
                }
                i = j;
                break;
            }
            else
            {
                char temp[2] = { str[j], '\0' };
                strcatt(arr[strRealCnt], temp);
            }
        }
        for (size_t strsCnt = 0; strsCnt < strsPreCnt; strsCnt++)
        {
            free(arr[strsCnt]);
        }
        free(arr);
    }
    return result;
}

#endif // !STRING_PROCESSOR